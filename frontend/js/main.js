...
const UIController = {
  ...
  // ✨ FIX: Smart Auth UI updater that finds buttons dynamically!
  updateAuthUI() {
    const token = localStorage.getItem('desideal_token');
    ...
    // Dynamically hunt for the sidebar list items that contain the words "Sign In" and "Sign Out"
    const sidebarItems = Array.from(document.querySelectorAll('#sidebar ul li'));
    const sidebarSignIn = document.getElementById('sidebar-signin-link') || sidebarItems.find(li => li.innerHTML.includes('showAuthModal'));
    const sidebarSignOut = document.getElementById('sidebar-signout-link') || sidebarItems.find(li => li.innerHTML.includes('handleSignOut'));
    ...
    if (token && userName) {
      ...
      if (sidebarSignOut) sidebarSignOut.style.display = 'block';
    } else {
  ...
  renderProducts() {
    ...
    setTimeout(() => {
      ...
      if (loadWrap && loadBtn) {
        ...
        loadBtn.innerHTML = `Load More (${allProducts.length - AppState.displayedCount} more) <i class="fas fa-chevron-down"></i>`;
      }
  ...
  updateCartUI() {
    ...
    if (subtotalEl) {
        ...
        if(discRow) discRow.classList.toggle('hidden', AppState.promoDiscount === 0);
  ...
  startHeroSlider() { 
      if(document.getElementById('hero-slider')) {
          setInterval(() => window.changeSlide(1), CONFIG.SLIDER_DELAY); 
      }
  },
...
window.performSearch = () => {
  ...
  AppState.displayedCount = 12;
  const title = document.getElementById('products-title');
  if(title) title.textContent = `Search results for "${AppState.currentSearch}"`;
  UIController.renderProducts();
  const sec = document.getElementById('products-section');
  if(sec) sec.scrollIntoView({ behavior: 'smooth' });
};
...
// ✨ BULLETPROOF PAYMENT SELECTOR LOGIC
window.selectPaymentOption = (method) => {
  // 1. Uncheck all radios & remove selected class
  ...
  // 2. Check the specific radio & add selected class
...
window.placeOrder = async () => {
  ...
  if (!selectedRadio) return UIController.showToast('Please select a payment method.', 'error');
  ...
  if (selectedPaymentMethod === 'desideal') {
    UIController.showToast('<i class="fas fa-wallet"></i> Insufficient DesiDeal Balance. Please select Card, UPI, or COD.', 'error');
    return; 
  }
  ...
  if (selectedPaymentMethod === 'cod') {
    try {
      ...
      if (data.success) {
        ...
        AppState.cart = []; localStorage.setItem('desideal_cart', JSON.stringify([])); UIController.updateCartUI();
      } else { UIController.showToast(data.message, 'error'); }
    } catch (error) { UIController.showToast('<i class="fas fa-wifi"></i> Failed to place COD order.', 'error'); }
    return; 
  }
      ...
      handler: async function (response) {
        const verifyRes = await fetch('http://localhost:5000/api/orders/verify-and-save', {
          ...
          body: JSON.stringify({ 
            ...
            razorpay_signature: response.razorpay_signature, items: AppState.cart, total: totals.total, shipping: shippingInfo
          })
        });
  ...
  } catch (error) { UIController.showToast('Payment system error.', 'error'); }
};
